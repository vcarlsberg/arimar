...
for(x in c(1:12))
{
  ...
  testFun <- function(x)
  {
    svm.model.tuning <- svm(x=c(1:length(myts)),y=residual.data1[1:length(myts)],
                            kernel="radial",gamma=2^x[1],cost = 2^x[2])
    
    mae(residual.data1,svm.model.tuning$fitted)
  }
  ...
  if(x==1)
  {
    ...
    svm.model <- svm(x=c(1:length(myts)),y=residual.data1[1:length(myts)],
                     kernel="radial",
...
points(arima.model$fitted, col="red", pch="*")
